...
export default function PricingModal({
  ...
  async function payWithTelegram() {
    ...
    setPaying(true);
    try {
      const res = await fetch("/api/telegram/stars/invoice-link", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ planId: plan.id, templateTitle: video?.title }),
      });
      const json = (await res.json().catch(() => ({}))) as { url?: string; error?: string };
      if (!res.ok || !json.url) throw new Error(json.error || "Failed to create invoice link");
          ...
          <div className="mt-4 rounded-xl border border-white/[0.08] bg-white/[0.02] p-3.5">
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                {paying ? "OPENING TELEGRAM..." : "CARD WITH TELEGRAM"}
              </button>
            ...
            {payError && (
              <p className="mt-2 text-center text-rose-300 text-[11px] font-semibold">
                {payError}
              </p>
            )}
          </div>
